...
import com.fleetops.dto.DriverUpdateRequest;
...
class DriverControllerTest {
    ...
    class CrudTests {
        ...
        void getById() throws Exception {
            ...
            mockMvc.perform(get("/api/drivers/1").with(TestAuth.auth())).andExpect(status().isOk())
        ...
        void getById_NotFound() throws Exception {
            ...
            mockMvc.perform(get("/api/drivers/404").with(TestAuth.auth())).andExpect(status().isNotFound())
        ...
        void list() throws Exception {
            ...
            mockMvc.perform(get("/api/drivers/list").with(TestAuth.auth())).andExpect(status().isOk())
                   .andExpect(header().string("Content-Type", containsString(MediaType.APPLICATION_JSON_VALUE)))
        ...
        void create() throws Exception {
            ...
            mockMvc.perform(post("/api/drivers").with(TestAuth.auth()).with(csrf()).contentType(MediaType.APPLICATION_JSON)
                                                .content(objectMapper.writeValueAsString(payload)))
                   .andExpect(status().isCreated()).andExpect(header().string("Location", endsWith("/api/drivers/10")))
                   .andExpect(header().string("Content-Type", containsString(MediaType.APPLICATION_JSON_VALUE)))
                   .andExpect(jsonPath("$.id").value(10)).andExpect(jsonPath("$.name").value("New"))
        ...
        void update() throws Exception {
            ...
            mockMvc.perform(put("/api/drivers/5").with(TestAuth.auth()).with(csrf()).contentType(MediaType.APPLICATION_JSON)
                                                 .content(objectMapper.writeValueAsString(patch)))
                   .andExpect(status().isOk())
                   .andExpect(header().string("Content-Type", containsString(MediaType.APPLICATION_JSON_VALUE)))
                   .andExpect(jsonPath("$.id").value(5)).andExpect(jsonPath("$.name").value("Updated"));
        ...
        void deleteDriver() throws Exception {
            ...
            mockMvc.perform(delete("/api/drivers/7").with(TestAuth.auth()).with(csrf())).andExpect(status().isNoContent()).andExpect(content().string(""));
        ...
        void update_NotFound() throws Exception {
            ...
            patch.setName("Updated");
            given(driverService.update(eq(404L), any(Driver.class))).willThrow(
                    new DriverNotFoundException("Driver not found"));
            mockMvc.perform(put("/api/drivers/404").with(TestAuth.auth()).with(csrf()).contentType(MediaType.APPLICATION_JSON)
                                                   .content(objectMapper.writeValueAsString(patch)))
        ...
        void update_InvalidPayload() throws Exception {
            String invalidJson = "{}";
            mockMvc.perform(put("/api/drivers/5").with(TestAuth.auth()).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(invalidJson))
        ...
        void deleteDriver_NotFound() throws Exception {
            ...
            org.mockito.Mockito.doThrow(new DriverNotFoundException("Driver not found")).when(driverService)
                               .delete(404L);
            mockMvc.perform(delete("/api/drivers/404").with(TestAuth.auth()).with(csrf())).andExpect(status().isNotFound())
                   .andExpect(jsonPath("$.status").value(404)).andExpect(jsonPath("$.error").value("Not Found"))
                   .andExpect(jsonPath("$.message").value(containsString("Driver not found")));
        }
    }
    ...
    class AuthTests {
        ...
        void getDriver_shouldReturn401_whenNoJwtProvided() throws Exception {
            mockMvc.perform(get("/api/drivers/1")).andExpect(status().isUnauthorized());
        }

        @Test
        @DisplayName("getDriver_shouldReturn403_whenJwtLacksRole")
        void getDriver_shouldReturn403_whenJwtLacksRole() throws Exception {
            // Arrange: make service return something if controller is executed (so we can detect it)
            given(driverService.getById(1L)).willReturn(Driver.builder().id(1L).name("John").build());

            // Act: perform with a restricted role
            mockMvc.perform(get("/api/drivers/1").with(TestAuth.auth("ROLE_RESTRICTED")))
                   // Assert: should be forbidden
                   .andExpect(status().isForbidden());

            // Ensure service was not invoked when access denied
            Mockito.verify(driverService, Mockito.never()).getById(1L);
        }

        @Test
        @DisplayName("getDriver_shouldReturn200_whenJwtHasUserRole")
        void getDriver_shouldReturn200_whenJwtHasUserRole() throws Exception {
            mockMvc.perform(get("/api/drivers/1").with(TestAuth.auth())).andExpect(status().isOk());
        }
    }
    ...
    class InputValidationTests {
        ...
        void createDriver_shouldReturn400_whenInvalidPayload() throws Exception {
            ...
            mockMvc.perform(post("/api/drivers").with(TestAuth.auth()).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(invalidJson))
    ...
    class ErrorHandlingTests {
        ...
        void getDriver_shouldReturn404_whenDriverNotFound() throws Exception {
            ...
            given(driverService.getById(999L)).willThrow(
                    new com.fleetops.exception.DriverNotFoundException("Not found"));
            // Act: Perform GET request
            mockMvc.perform(get("/api/drivers/999").with(TestAuth.auth()))
